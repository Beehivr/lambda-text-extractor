...
__name__ = 'ocr_extractor'

def extract(event_handler, context, ext=None):
    global logger
    ...
    searchable_pdf_uri = event_handler.key + '.searchable.pdf'
    ...
    logger.info('{} invoked with event {}.'.format(AWS_LAMBDA_FUNCTION_NAME, json.dumps(event_handler.event)))
    ...
    with NamedTemporaryFile(mode='wb', suffix=ext, delete=False) as f:
        ...
        logger.debug('Downloaded <{}> to <{}>.'.format(event_handler.key, document_path))
    #end with
    ...
    try:
        textractor_results = extract_func(document_path=document_path, event_handler=event_handler, context=context, create_searchable_pdf=create_searchable_pdf)
        ...
        if searchable_pdf_path:
            ...
            with open(searchable_pdf_path, 'rb') as f:
                ...
                logger.debug('Searchable PDF version of <{}> saved to <{}>.'.format(event_handler.key, searchable_pdf_uri))
            #end with
        ...
        if page: logger.debug('Extracted page {} of <{}> to <{}> (took {:.3f} seconds).'.format(page, event_handler.key, text_uri, time.time() - start_time))
        else: logger.debug('Extracted pages of <{}> to <{}> (took {:.3f} seconds).'.format(event_handler.key, text_uri, time.time() - start_time))

    except Exception as e:
        logger.exception('Extraction exception for <{}>'.format(event_handler.key))
...
def pdf_to_text_with_ocr(document_path, event_handler, context, page=None, create_searchable_pdf=True):
    global logger
    ...
    temp_uri_prefix = event_handler.change_file_extension('.tmp')

    if page is not None:
        return pdf_to_text_with_ocr_single_page(document_path, create_searchable_pdf=create_searchable_pdf)
    ...
    with closing(asyncio.new_event_loop()) as event_loop:
        ...
        async def _invoke_textract_ocr_tasks(_completed_text_contents, _completed_searchable_pdf_fnames, timeout):
            ...
            for page in range(1, num_pages + 1):
                ...
                logger.info('>>>>>>>>>>> file name: ' + page_text_uri)

                payload = dict(event_handler=event_handler, page=page)
        ...
        textract_output_wait_timeout=1
    ...
    if create_searchable_pdf:
        try:
            ...
            searchable_pdf_path = None
            logger.warning('Timeout while merging searchable PDF for <{}>.'.format(event_handler.key))

        except Exception as e:
            logger.exception('Exception while merging searchable PDF for <{}>.'.format(event_handler.key))

        finally:
            for fname in pdf_pages_filenames:
                try: os.remove(fname)
                except Exception as e: logger.exception('searchable_pdf_remove_exception', filename=fname, event_handler=event_handler)
            #end for
        #end try
    #end if
    ...
    if missing_text_pages:
        logger.info('Missing pages {} in <{}>.'.format(missing_text_pages, event_handler.key))
        meta['missing_text_pages'] = ','.join(str(p) for p in missing_text_pages)
    #end if

    if empty_content_pages:
        logger.info('Empty content pages {} in <{}>.'.format(empty_content_pages, event_handler.key))
        meta['empty_content_pages'] = ','.join(str(p) for p in empty_content_pages)
    #end if

    if missing_searchable_pdf_pages:
        logger.info('Missing searchable PDF pages {} in <{}>.'.format(missing_searchable_pdf_pages, event_handler.key))
...
def pdf_to_text_with_ocr_single_page(document_path, event_handler, context, create_searchable_pdf=True):
    # page = event['page']
    page = None
...
def image_to_text(document_path, event_handler, context, create_searchable_pdf=True):
